...
int check_state(int T, int state[4], int past_state[SearchMax][4]) {
    ...
    /*** この部分を自分で作成する ***/
    if (state[WOLF] == 1 && state[GOAT] == 1 && state[MAN] == 0) {
        return 0;
    } else if (state[GOAT] == 1 && state[CABBAGE] == 1 && state[MAN] == 0) {
        return 0;
    }
    ...
    for (i = 0; i < 4; i++) { /* 0: 男 1: 狼 2: 山羊 3: キャベツ を順に調べる */
        if (src_state[i] == 1) { /* 移動できるのであれば(男と同じ側にいるのであれば) */
            ...
            if (check_state(T, new_src_state, src_side)) {
                ...
                if (/*** この部分を自分で作成する ***/
                        right_side[T + 1][MAN] == 1 && right_side[T + 1][WOLF] == 1 &&
                        right_side[T + 1][CABBAGE] == 1 && right_side[T + 1][GOAT] == 1) {
                    print_ans(T + 1);
                    exit(0);
                }
                    /* そうでなければ再帰的に探索を続ける */
